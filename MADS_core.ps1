...
$scriptName + ": Loaded Room: room_" + $args[0] + ".ini" | out-file $env:temp\updater.log -Append -encoding ASCII
...
foreach ($i in $scripts) {
	...
	if (Test-Path $modules$i\$i.ini) {
		if ((Test-Path ($extensions + "Shim.ps1")) -Or (Test-Path ($modlets + "Shim.ps1"))){
			...
			$result = Start-Process cmd.exe -ArgumentList "/C title MADS_module: ", $i,"& Powershell.exe -ExecutionPolicy Bypass -File Shim.ps1 /module ", $i, "/mode run" -Wait -PassThru
			$exists = $TRUE
